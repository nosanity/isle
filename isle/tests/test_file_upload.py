...
class BaseUpload:
    ...
    def setUp(self):
        global_context = Context.objects.create(
            timezone='Europe/Moscow', uuid=str(uuid4()), title='global', guid='global',
            status=Context.STATUS_ACTIVE, ct_type=Context.TYPE_OPEN, is_global=True
        )
        context1 = Context.objects.create(
            timezone='Europe/Moscow', uuid=str(uuid4()), title='context1', guid='context1',
            status=Context.STATUS_ACTIVE, ct_type=Context.TYPE_OPEN, parent=global_context
        )
        ...
        self.assistant_tag = Tag.objects.create(slug=settings.ASSISTANT_TAGS_NAME[0])
